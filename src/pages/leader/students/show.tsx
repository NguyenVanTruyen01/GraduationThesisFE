...
interface ILeaderStudentShowProps {
}
...
const LeaderStudentShow: React.FunctionComponent<ILeaderStudentShowProps> = (props) => {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data: any) => {
    try {
      ...
      const response = await axios.patch(`${BASE_API_URL}/users/leader/updateById/${studentID}`, payload, { headers })
  ...
  const fetchCurrentUser = useCallback(async () => {
    const response = await axios.get(`${BASE_API_URL}/users/leader/findById/${studentID}`, { headers });
    if (response.data.statusCode === 200) {
...
export default LeaderStudentShow;
