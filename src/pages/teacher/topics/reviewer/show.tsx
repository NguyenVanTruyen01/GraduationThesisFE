...
const TeacherReviewTopicsShow = (props: any) => {
  ...
  // useEffect(() => { console.log(groupStudents); }, [groupStudents])
  ...
  const actionBodyTemplate = (group: any) => {
    return (
      <div>
        <span className="pi pi-eye hover:text-lime-500 cursor-pointer" onClick={() => navigate(`/teacher/group/students/${group._id}`)} />
      </div>
      ...
      <div className="flex flex-col gap-3" >
        <p className={`${showStyle.title} item-btw`}>
          <Label>Giảng viên hướng dẫn &nbsp; &nbsp; &nbsp;
            <i className="fa-regular fa-credit-card cursor-pointer" onClick={() => handleOpenMoreInfo(topicDetail?.topic_instructor)}></i>
          </Label>
        </p>
        ...
        <p className={showStyle.nameReviewer}>
          {topicDetail !== null ?
            <>
              {topicDetail?.topic_instructor?.user_name}
            </> : "Chưa có thông tin"}
        </p>
      </div>
...
export default TeacherReviewTopicsShow;
