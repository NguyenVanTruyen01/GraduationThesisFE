...
import DetailHistoryTopic from "./DetailHistoryTopic";
...
const TeacherTopicsPendingList: React.FC<ITopicsPendingProps> = (props) => {
  ...
  const headerExport = ["Tên Đề Tài", "Tên giảng viên", "Số lượng", "Status"];
  const body = [
    ["Edison", "Padilla", 14],
    ["Cheila", "Rodrigez", 56],
  ];
  ...
  const onRowSelect = (event: DataTableSelectEvent) => {
    ...
    setShowDetail(true)
    navigate(`/teacher/history/topics/${event.data?._id}`)
    // navigate()
  ...
  async function fetchData(filterData: any) {

    const response = await axios.post(`${BASE_API_URL}/topics/teacher/getTopicByFilter`, filterData, { headers })
    const period = await axios.get(`${BASE_API_URL}/registration_period/teacher/getAll`, { headers })
  ...
  const header = (
    <div className="flex flex-wrap items-center justify-between ">
      <span className="text-xl text-900 font-bold">Đề tài đã hướng dẫn qua các năm</span>
