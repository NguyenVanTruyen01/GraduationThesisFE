...
import ResultMentor from "../mentor/FromScore"

import FormScore from "../mentor/FormScoreReviewer"
...
const DetailHistoryTopic: React.FC<IDetailTopicsProps> = (props) => {
        ...
        {group?.group_member.map((member: any, index: number) => {
          return (
            <div key={index} className="item-btw">
              ...
              <button onClick={() => handleOpenResultMentor(member?._id)} className={styles.scoreBtn}><i className="fa-solid fa-forward fa-beat-fade"></i> &nbsp; Điểm hướng dẫn</button>
            </div>
          )
        })}
      </div>
        ...
        {group?.group_member.map((member: any, index: number) => {
          return (
            <div key={index} className="item-btw">
              <div>{member.user_name}  </div>
              <div>{member.user_id}  </div>
              <button  onClick={() => handleOpenResultAssembly(member?._id)} className={styles.scoreBtn}><i className="fa-solid fa-forward fa-beat-fade"></i> &nbsp; Điểm phản biện</button>
              {/* <Button size="small" icon="pi pi-pencil" raised onClick={()=>handleOpenResultAssembly(member?._id)} /> */}
            </div>
          )
        })}
      </div>
  ...
  const actionBodyTemplate = (group: any) => {
    return (
      <div>
        {/* <Button size="small" icon="pi pi-eye" raised onClick={() => navigate(`/teacher/group/students/${group._id}?topicID=${topicID}`)} /> */}
        <button onClick={() => navigate(`/teacher/group/students/${group._id}?topicID=${topicID}`)} className={styles.scoreBtn}><i className="fa-regular fa-eye"></i></button>
      </div>
    );
  };
  ...
  const header = (
    <div className="flex flex-wrap items-center justify-between ">
      <span className="text-base text-900 font-bold">Nhóm đăng ký</span>
    </div>
  );
  ...
  return (
    <section className="card m-[1rem] flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-3" >
        <p className={`${showStyle.title} item-btw`}>
          <Label>Giảng viên phản biện &nbsp; &nbsp; &nbsp;
            {
              topicDetail !== null && topicDetail?.topic_reviewer ?
      ...
      <DataTable
        className="text-sm"
        value={groupStudents}
        removableSort header={
          <div className="flex flex-wrap items-center justify-between ">
            <span className="text-base text-900 font-bold">Điểm phản biện</span>
          </div>}
      ...
      >
        ...
        <Column style={{ maxWidth: 150 }} sortable body={rowNumberTemplate} header="STT" />
        <Column style={{ maxWidth: 150 }} field="group_member.length" align="center" sortable header="Số lượng" />
        <Column header="Thành viên" body={memberBodyTemplateAssembly} />
        <Column header="Xem chi tiết nhóm" align="center" body={actionBodyTemplate} />
      </DataTable>
      <br />
      <DataTable
        className="text-sm"
      ...
      {openFormScoreAssembly ? <FormScore handleClose={handleCloseResultAssembly} id={idStudent} topic={topicDetail} /> : <></>}
      {
        openMoreInfo ?
...
export default DetailHistoryTopic;
